...
type WorkTab = "info" | "cover" | "chapters" | "glossary" | "series" | "pricing";
 ...
 */
function GlossaryTab({ novel }: { novel: WriterNovel }) {
  const qc = useQueryClient();
  ...
  const glossary = useQuery({
    queryKey: ["writer-glossary", novel.id],
    queryFn: () => api.listWriterGlossary(novel.id),
  });

  const invalidate = () => qc.invalidateQueries({ queryKey: ["writer-glossary", novel.id] });

  const removeEntry = useMutation({
    mutationFn: (id: string) => api.deleteGlossaryEntry(id),
    onSuccess: () => {
  ...
  const removeGroup = useMutation({
    mutationFn: (id: string) => api.deleteGlossaryGroup(id),
...
function GlossaryGroupSheet({
  ...
  const submit = useMutation({
    mutationFn: () => api.createGlossaryGroup(novelId, name.trim(), sortNo),
    onSuccess: () => {
      onSaved();
      onClose();
    },
...
function GlossaryEntrySheet({
  ...
  const submit = useMutation({
    mutationFn: () => {
      ...
      return entry
        ? api.updateGlossaryEntry(entry.id, payload)
        : api.createGlossaryEntry(novelId, { group_id: groupId, ...payload });
    },
    onSuccess: () => {
      onSaved();
      onClose();
