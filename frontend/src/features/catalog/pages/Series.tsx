...
import { useSeriesFollow, useToggleSeriesFollow } from "@/features/social";
import { numberTH } from "@/shared/lib/format";
import { Empty, ErrorNote, Loading, NovelCover } from "@/shared/ui";

import type { RelatedNovel, SeriesBook } from "../api";
import { useRelatedNovels, useSeries } from "../queries";
...
function FollowSeries({ seriesId }: { seriesId: string }) {
  ...
  const state = useSeriesFollow(seriesId, Boolean(user));
  const toggle = useToggleSeriesFollow(seriesId);
  ...
  return (
    <div>
      <button
        ...
        onClick={() => toggle.mutate(current !== "all")}
