...
import { useShelf } from "@/features/library";
import { greeting, numberTH, percent, relativeTime } from "@/shared/lib/format";
import { Empty, Loading, NovelCover } from "@/shared/ui";

import { NovelCard } from "../components/NovelCard";
import { useNovels, useWeeklyRanking } from "../queries";

export default function Home() {
  const { user } = useAuth();

  const latest = useNovels({ sort: "latest", limit: 8 });
  const ranking = useWeeklyRanking(5);
  const shelf = useShelf("reading", Boolean(user));
