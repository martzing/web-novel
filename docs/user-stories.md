...
Status: ✅ built and covered by a test · ❌ not implemented. The four ❌ rows are
the admin console, which `README.md` and `docs/api-spec.md` also list as out of
scope — only `POST /admin/wallet-adjust` exists, because the coin test matrix
needs it.
...
| R-17 | As a reader, I can follow a novel and get notified of new chapters.                                                | Notification row created on chapter publish, and surfaced by the bell in the app shell with an unread count. | ✅ |
...
| W-05 | Manage per-novel glossary (groups + entries).                                 | Full CRUD from the `อภิธานศัพท์` tab. Deleting a term bumps `glossary_rev` so bound chapters re-render and the marker degrades to plain text; deleting a group that still holds terms is refused with `409 GROUP_NOT_EMPTY`. | ✅ |
...
| W-08 | View stats: reads, followers, coins and `อ่านจบต่อบท`, over 14 / 30 days.      | KPI matches sum from `chapter_daily_stats`; the completion rate is that window's completions ÷ reads. | ✅ |
...
| W-11 | Manage all my works from one screen, grouped by series.                       | จัดการผลงาน lists every owned novel under its series; selecting one loads its six tabs (the design's five, plus `อภิธานศัพท์`). | ✅ |
...
| ID   | Story                                                              | Acceptance | Status |
| ---- | ------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------ |
| R-20 | See how far a translation has to run.                              | The detail and series pages show `บทที่แปลแล้ว` beside `บทในต้นฉบับ`; the shelf reads progress against the translated count, which is the figure that answers "how much is left for me". | ✅ |
...
| R-29 | Follow every book in a series in one click.                        | `ติดตามทั้งชุด` fans out over per-novel follows; the button reads none / partial / all, and a book that joins later leaves the series `partial` rather than following itself. | ✅ |
